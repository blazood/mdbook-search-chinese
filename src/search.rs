...
pub fn create_files(search_config: &Search, destination: &Path, book: &Book) -> Result<()> {
    // add dynamic language config
    
    let mut index = Index::with_language(Language::from_code(search_config.language.as_str()).unwrap() ,&["title", "body", "breadcrumbs"]);
